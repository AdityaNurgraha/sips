...
@section('title', 'Tambah List Pelanggaran (Guru)')
    ...
    <form action="{{ route('guru.pelanggaran.store') }}" method="POST" class="space-y-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div>
                ...
                <select id="kategori-select" class="w-full border rounded px-3 py-2 focus:ring focus:ring-blue-200">
                    ...
                    @foreach($kategori->groupBy('kategori') as $kategoriNama => $items)
                        <option value="{{ $kategoriNama }}">{{ $kategoriNama }}</option>
                    @endforeach
                </select>
            </div>
            ...
            <div>
                ...
                <select name="kategori_pelanggaran_id" id="pelanggaran-select" class="w-full border rounded px-3 py-2 focus:ring focus:ring-blue-200" required>
        ...
        <div class="flex justify-end space-x-2 mt-4">
            <button type="reset" class="px-4 py-2 bg-red-500 text-white rounded hover:bg-red-600 transition">Reset</button>
            <button type="submit" class="px-4 py-2 bg-blue-500 text-white rounded hover:bg-blue-600 transition">Submit</button>
        </div>
    </form>
</div>
...
document.addEventListener('DOMContentLoaded', function () {
    ...
    // Auto-fill NIS saat pilih siswa
